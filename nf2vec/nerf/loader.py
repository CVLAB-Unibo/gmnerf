...
class NeRFLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.camtoworlds, self.focal = load_renderings(data_dir, split, self.HEIGHT, self.WIDTH)
        self.camtoworlds = (torch.from_numpy(self.camtoworlds).to(self.device).to(torch.float32))
        self.K = torch.tensor(
            [
...
def load_renderings(data_dir: str, split: str, h: int, w: int):
    with open(os.path.join(data_dir, "transforms_{}_compressed.json".format(split)), "r") as fp:
        meta = json.load(fp)
