...
from nerfacc import OccupancyGrid, contract_inv, ray_marching, rendering
...
def render_image_from_decoder(
    ...
    chunk = (
        ...
        if decoder.training or batch_size > 1
        else 1024
    )
    ...
    for i in range(0, num_rays, chunk):
            ...
            if decoder.training or batch_size > 1:
                for batch_idx in range(batch_size):
                    ...
                    # TRUNCATION: Randomly sample MAX_SIZE elements from the tensor
                        ...
                        indices, _ = torch.sort(indices)  # this is important to avoid issues with volume rendering
        ...
        curr_rgb, curr_sigmas = decoder(embeddings, b_positions)
    ...
    return colors, opacities, depths, n_rendering_samples, bg_rgb_pred, bg_rgb_label


@torch.no_grad()
def render_image_from_nerf(
    # scene
    ...
    radiance_field = copy.deepcopy(radiance_field)
    occupancy_grid = copy.deepcopy(occupancy_grid)
    
    input_radiance_field = radiance_field
    field_is_dict = isinstance(radiance_field, dict)
    ...
    def sigma_fn(t_starts, t_ends, ray_indices):
        ...
        _, density = radiance_field.query_density_and_rgb(positions, None)
        return density
    ...
    for batch_idx in range(batch_size):
        curr_bkgd = color_bkgds if color_bkgds is None else color_bkgds[batch_idx]
        curr_grid_weights = {
            "_roi_aabb": grid_weights["_roi_aabb"][batch_idx],
            "_binary": grid_weights["_binary"][batch_idx],
            "resolution": grid_weights["resolution"][batch_idx],
        ...
        encoding_params = ngp_mlp_weights["encoding.params"][batch_idx]
        mlp_weights = ngp_mlp_weights["mlp_base.params"][batch_idx]
        encoding_params = encoding_params[~torch.isnan(encoding_params)]
        mlp_weights = mlp_weights[~torch.isnan(mlp_weights)]
        curr_ngp_mlp_weights = {
            "encoding.params": encoding_params,
            "mlp_base.params": mlp_weights
        }
        try:
            is_triplane = ngp_mlp_weights["is_triplane"][batch_idx]
        except KeyError:
            is_triplane = "triplane" in ngp_mlp_weights
        if is_triplane:
            curr_ngp_mlp_weights["triplane"] = ngp_mlp_weights["triplane"][batch_idx]
        
        if field_is_dict:
            if ngp_mlp_weights["is_triplane"][batch_idx]:
                radiance_field = input_radiance_field["triplane"]
            elif ngp_mlp_weights["is_hash"][batch_idx]:
                radiance_field = input_radiance_field["hash"]
            else:
                radiance_field = input_radiance_field["mlp"]

        radiance_field.load_state_dict(curr_ngp_mlp_weights, strict=False)
